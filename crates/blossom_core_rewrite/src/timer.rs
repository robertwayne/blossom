...
impl Timer {
    ...
    /// Updates the timer last_action field using the current time.
    pub fn update(&mut self) {
        self.last_action = Instant::now().duration_since(self.start_time).as_secs();
    }

    /// Advances the timer by one server tick. This will put the main loop to
    /// sleep for the interval defined in the game config file.
