...
// When set, the client side of the connection should process all input lines, performing any
// editing function, and only send completed lines to the remote side. When unset, client side
// should not process any input from the user, and the server side should take care of all
// character processing that needs to be done.
pub const LINEMODE_EDIT: u8 = 1;

// When set, the client side should translate appropriate interrupts/signals to their Telnet
// equivalent. (These would be IP, BRK, ABORT, EOF, and SUSP). When unset, the client should pass
// interrupts/signals as their normal ASCII values.
